...
export function QuickCapture() {
  ...
  // ⌘K / Ctrl-K from anywhere. Esc, the focus trap and focus restoration all
  // live in <Dialog> now — they were duplicated here first, which is exactly
  // why they became a primitive.
  useEffect(() => {
  ...
  // Focus restoration is <Dialog>'s job now — this only resets our own state.
  function close() {
    setIsOpen(false);
    setText("");
    setDraft(null);
  ...
  const isLowConfidence =
    draft !== null && draft.confidence < CONFIDENCE_THRESHOLD;

  return (
    <>
      <button
        onClick={() => setIsOpen(true)}
        className="fixed bottom-6 right-6 z-40 rounded-full bg-[var(--primary)] px-5 py-3 text-sm font-medium text-[var(--primary-contrast)] shadow-[var(--shadow-lg)] sm:bottom-6"
      >
        ＋ Capture
        <kbd className="ml-2 hidden text-xs opacity-70 sm:inline">⌘K</kbd>
      </button>

      <Dialog isOpen={isOpen} onClose={close} label="Quick capture">
          ...
          {!draft && (
            <div className="mt-3 flex items-center justify-between">
              <p className="text-xs text-[var(--text-muted)]">
                Try &ldquo;swiggy 480 yesterday&rdquo; or &ldquo;salary
                85000&rdquo;
              </p>
